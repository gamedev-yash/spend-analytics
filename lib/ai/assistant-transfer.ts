...
import type { DashboardKey } from "./dashboard-registry";
...
/** Reads and clears the transferred state, if any — meant to be consumed exactly once, and only by the dashboard it was captured from (a mismatched key is discarded rather than bleeding into the wrong dashboard's transcript). */
export function takeTransferState(dashboardKey: DashboardKey): AssistantTransferState | null {
  ...
  try {
    ...
    return parsed.dashboardKey === dashboardKey ? parsed : null;
  } catch {
    return null;
  }
}
