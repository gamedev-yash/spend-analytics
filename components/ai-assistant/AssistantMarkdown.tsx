  ...
  return (
    <div className={cn("space-y-2", className)}>
      {blocks.map((block, i) => {
        ...
        return (
          <div key={key} className="overflow-x-auto rounded-md border border-slate-200 dark:border-slate-700">
            <table className="w-full min-w-full text-left text-xs">
              <thead className="bg-slate-100 dark:bg-slate-800/80">
                <tr>
                  {block.header.map((cell, ci) => (
                    <th key={ci} className="whitespace-nowrap px-2.5 py-1.5 font-semibold text-slate-700 dark:text-slate-200">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {block.rows.map((row, ri) => (
                  <tr key={ri} className="even:bg-slate-50/70 dark:even:bg-slate-800/40">
                    {row.map((cell, ci) => (
                      <td key={ci} className="whitespace-nowrap px-2.5 py-1.5 text-slate-600 dark:text-slate-300">
